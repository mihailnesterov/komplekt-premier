...
if ( ! function_exists( 'storefront_before_content' ) ) {
	...
	function storefront_before_content() {
		...
		<div id="primary" class="content-area">
			<main id="main" class="site-main 000" role="main">
		<?php
	}
}
	...
	function storefront_cart_link() {
		?>
			<a class="cart-contents" href="<?php echo esc_url( WC()->cart->get_cart_url() ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'storefront' ); ?>">
				<span class="amount">
				<?php echo "Корзина";/** /echo wp_kses_data( WC()->cart->get_cart_subtotal() );/**/ ?>
	...
	function storefront_product_search() {
		if ( storefront_is_woocommerce_activated() ) { ?>
			...
			<div class="site-search" style="display:none;">	<!-- hidden by Mihail 26-09-19 -->
				...
				<div class="shop_login <?php if(!is_user_logged_in()){ ?> login<?php } ?>">
				  <?php if ( is_user_logged_in() ) { ?>
					  <nobr><a href="/my-account" class="account_link">Личный кабинет</a></nobr>
					  <nobr><a href="http://komplekt-premier.ru/my-account/customer-logout/" class="logout_link">Выйти</a></nobr>
				  <?php } else { ?>
					  <a href="#contact_form_pop" class="fancybox-inline shop_login_button">Вход/регистрация</a>
				  <?php }?>
							...
							<div class="u-columns col2-set" id="customer_login">
								<div class="u-column1 col-1">
									...
									<form method="post" class="login">
										...
										<p class="form-row">
											<input type="hidden" id="woocommerce-login-nonce" name="woocommerce-login-nonce" value="da0ac7b363"><input type="hidden" name="_wp_http_referer" value="/my-account/"><input type="submit" class="woocommerce-Button button" name="login" value="Авторизация">
											<label for="rememberme" class="inline">
												<input class="woocommerce-Input woocommerce-Input--checkbox" name="rememberme" type="checkbox" id="rememberme" value="forever"> Запомнить меня</label>
										</p>
										<p class="woocommerce-LostPassword lost_password">
											<a href="http://komplekt-premier.ru/my-account/lost-password/">Забыли свой пароль?</a>
										</p>
									</form>
								</div>
									...
									<form method="post" class="register">
										<p class="woocommerce-FormRow woocommerce-FormRow--wide form-row form-row-wide">
											...
											<p class="woocommerce-FormRow woocommerce-FormRow--wide form-row form-row-wide">
												...
												<input type="password" class="woocommerce-Input woocommerce-Input--text input-text" name="password" id="reg_password"><div class="woocommerce-password-strength bad" aria-live="polite">Слабый - Пожалуйста, введите более надёжный пароль.</div><small class="woocommerce-password-hint">Пароль должен иметь длину не менее семи символов. Чтобы сделать его надёжнее, используйте буквы в верхнем и нижнем регистре, цифры и спецсимволы, как ! " ? $ % ^ &amp; ).</small>
											</p>
										...
										<p class="woocomerce-FormRow form-row">
											<input type="hidden" id="woocommerce-register-nonce" name="woocommerce-register-nonce" value="e0ab67e592"><input type="hidden" name="_wp_http_referer" value="/my-account/">				<input type="submit" class="woocommerce-Button button disabled" name="register" value="Регистрация" disabled="disabled">
										</p>
									</form>
								</div>
							</div>
